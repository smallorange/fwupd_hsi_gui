...
class GccDBusTestCase(DBusTestCase):
    ...
    def __init__(self):
        ...
        os.environ['UDEV_LOG'] = 'debug'
        self.upowerd = subprocess.Popen([ self.upowerd_path, '-v' ],
    ...
    def cycle_degraded(self):
        perf = self.ppd.Get('PerformanceDegraded')
        if perf == '':
            perf = 'lap-detected'
            ...
            perf = ''
        dbusmock = dbus.Interface(self.ppd, dbusmock.MOCK_IFACE)
        dbusmock.SetDeviceProperties('/net/hadess/PowerProfiles', {
                'PerformanceDegraded': dbus.String(perf, variant_level=1)
        })

    def start_menu(self):
        # self.toggle_devices(['battery', 'ac'], '1')
        # self.toggle_devices(['battery', 'ac'], '1')
        # return
        # self.toggle_degraded()

        menu = ConsoleMenu("Power Panel", "Scenario Tester")
    ...
    def wrap_call(self):
        ...
        args = ['gnome-control-center', '-v', 'power']
        if wrapper == 'gdb':
